...
def get_market_stats() -> List[Dict[str, Any]]:
    """获取各市场数据统计"""
    ...
    for row in results:
        ...
        board_type = board_type or 'N/A'  # type: ignore
        stats.append({
            'market': market,
...
def repair_market_data(market: str, board_type: Optional[str] = None, batch_size: int = 20, max_stocks: int = 100):
    """修复指定市场的数据"""
...
def main():
    ...
    print("\n=== 当前数据覆盖情况 ===")
    for stat in stats:
        stat_dict: Dict[str, Any] = stat  # type: ignore
        print(f"{stat_dict['market']} {stat_dict['board_type']}: {stat_dict['coverage_rate']}% ({stat_dict['stocks_with_data']}/{stat_dict['total_stocks']})")
    ...
    for market, board_type in priority_markets:
        # 找到对应的统计信息
        market_stat = next((s for s in stats if s.get('market') == market and s.get('board_type') == board_type), None)
