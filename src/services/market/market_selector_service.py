...
try:
    ...
    logging.warning("无法导入UnifiedDatabaseManager，设置为None")

# 延迟导入避免循环导入问题
def get_intelligent_stock_selector():
    """延迟获取IntelligentStockSelector实例"""
    try:
        from ...apps.scripts.selector_service import IntelligentStockSelector
        return IntelligentStockSelector
    except ImportError:
        logging.warning("无法导入IntelligentStockSelector，设置为None")
        return None

def get_stock_status_filter():
    """延迟获取StockStatusFilter实例"""
    try:
        from ...services.stock.stock_status_filter import StockStatusFilter
        return StockStatusFilter
    except ImportError:
        logging.warning("无法导入StockStatusFilter，设置为None")
        return None
...
class MarketSelectorService:
    ...
    def __init__(self):
        if UnifiedDatabaseManager is None:
            raise ImportError("UnifiedDatabaseManager未找到，无法初始化MarketSelectorService")
        IntelligentStockSelector = get_intelligent_stock_selector()
        if IntelligentStockSelector is None:
            raise ImportError("IntelligentStockSelector未找到，无法初始化MarketSelectorService")
        StockStatusFilter = get_stock_status_filter()
        if StockStatusFilter is None:
            raise ImportError("StockStatusFilter未找到，无法初始化MarketSelectorService")
            
        self.db_manager = UnifiedDatabaseManager(db_type='mysql')
        ...
        try:
            with self.db_manager.get_conn() as conn:
                ...
                if not df.empty and df.iloc[0]['latest_date']:
                    latest_date = pd.to_DATE_FORMAT(df.iloc[0]['latest_date'], "%Y-%m-%d %H:%i:%s").date()
    ...
    def _fallback_stock_selection(self, 
        ...
        try:
            ...
            # 按评分排序并取前N只
            all_results.sort(key=lambda x: x['score'], reverse=1)
