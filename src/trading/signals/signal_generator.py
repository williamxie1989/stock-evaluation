...
class SignalGenerator:
    ...
    def _calculate_macd(self, prices: pd.Series, fast: int = 12, slow: int = 26, signal: int = 9) -> tuple:
        """计算MACD指标"""
        exp_fast = prices.ewm(span=fast, adjust=0).mean()
        exp_slow = prices.ewm(span=slow, adjust=0).mean()
        macd = exp_fast - exp_slow
        macd_signal = macd.ewm(span=signal, adjust=0).mean()
    ...
    def _filter_signals(self, signals: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
        ...
        for signal in signals:
            ...
            if last_signal_date:
                # 确保日期是datetime类型
                current_date = pd.to_DATE_FORMAT(signal['date'], "%Y-%m-%d %H:%i:%s")
                last_date = pd.to_DATE_FORMAT(last_signal_date, "%Y-%m-%d %H:%i:%s")
