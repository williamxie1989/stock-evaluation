...
from src.ml.training.v2.enhanced_trainer_v2 import EnhancedTrainerV2
...
def train_models(
    ...
    min_features: int = FEATURE_SELECTION_MIN_FEATURES,
    max_features: int = FEATURE_SELECTION_MAX_FEATURES,
    reuse_feature_selection: bool = FEATURE_SELECTION_CACHE_ENABLED,
    force_refresh_feature_selection: bool = FEATURE_SELECTION_CACHE_FORCE_REFRESH,
    feature_selection_cache_dir: str = FEATURE_SELECTION_CACHE_DIR,
    feature_selection_cache_ttl_days: int = FEATURE_SELECTION_CACHE_TTL_DAYS,
    ...
    max_features : int
        最多保留特征数
    reuse_feature_selection : bool
        是否复用历史特征选择结果
    force_refresh_feature_selection : bool
        是否强制忽略缓存重新计算
    feature_selection_cache_dir : str
        特征选择缓存目录
    feature_selection_cache_ttl_days : int
        特征选择缓存有效期（天）
    enable_optuna : bool
        是否启用Optuna优化
    optuna_trials : int
    ...
    if enable_feature_selection and train_classification:
        logger.info(f"✅ Stage 5: 特征选择已启用 (方法={feature_selection_method}, 范围={min_features}-{max_features})")
        if reuse_feature_selection:
            ttl_text = f"{feature_selection_cache_ttl_days}天" if feature_selection_cache_ttl_days else "无限期"
            logger.info(f"  ↳ 缓存：启用（目录={feature_selection_cache_dir}, TTL={ttl_text}）")
            if force_refresh_feature_selection:
                logger.info("  ↳ 本次执行将忽略历史缓存并重新生成")
        else:
            logger.info("  ↳ 缓存：禁用（每次重新计算特征选择）")
    if enable_optuna:
    ...
    trainer = EnhancedTrainerV2(
        ...
        config={
            ...
            'reuse_feature_selection': reuse_feature_selection,
            'force_refresh_feature_selection': force_refresh_feature_selection,
            ...
            'shap_sample_size': FEATURE_SELECTION_SHAP_SAMPLE_SIZE,
            ...
            # 数据驱动参数调整配置
            'enable_data_driven_param_adjustment': ENABLE_DATA_DRIVEN_PARAM_ADJUSTMENT,
            'data_driven_adjustment_mode': DATA_DRIVEN_ADJUSTMENT_MODE,
    ...
    train_cls = train_classification and len(unique_classes) >= 2

    if train_classification and not train_cls:
        logger.error(
...
def main():
    ...
    fs_group.add_argument('--feature-selection-method', type=str,
                         choices=['model_based', 'lightgbm', 'xgboost', 'random_forest', 'shap'],
                         default='model_based',
                         help="特征选择方法，可选 model_based/lightgbm/xgboost/random_forest/shap (default: model_based)")
    fs_group.add_argument('--min-features', type=int, default=FEATURE_SELECTION_MIN_FEATURES,
                         help=f'最少保留特征数 (default: {FEATURE_SELECTION_MIN_FEATURES})')
    fs_group.add_argument('--max-features', type=int, default=FEATURE_SELECTION_MAX_FEATURES,
                         help=f'最多保留特征数 (default: {FEATURE_SELECTION_MAX_FEATURES})')
    fs_group.add_argument('--feature-selection-threshold', type=str, default='median',
                         help='特征重要性阈值: median/mean/<float> (default: median)')
    fs_group.add_argument('--reuse-feature-selection', dest='reuse_feature_selection', action='store_true',
                         help='启用特征选择结果缓存复用')
    fs_group.add_argument('--no-reuse-feature-selection', dest='reuse_feature_selection', action='store_false',
                         help='禁用特征选择缓存，始终重新计算')
    fs_group.add_argument('--force-refresh-feature-selection', dest='force_refresh_feature_selection', action='store_true',
                          help='忽略历史缓存，强制重新生成本次特征选择结果')
    fs_group.add_argument('--feature-selection-cache-dir', type=str,
                         default=FEATURE_SELECTION_CACHE_DIR,
                         help='特征选择缓存目录 (default: artifacts/feature_selection)')
    fs_group.add_argument('--feature-selection-cache-ttl', type=int,
                         default=FEATURE_SELECTION_CACHE_TTL_DAYS,
                         help='特征选择缓存有效期（天），到期自动失效 (default: 14)')
    ...
    # 训练模型
    default_regression = ENABLE_REGRESSION_TASK if 'ENABLE_REGRESSION_TASK' in globals() else True
    if args.regression_only:
        train_cls_flag = False
        train_reg_flag = True
    elif args.classification_only:
        train_cls_flag = True
        train_reg_flag = False
    else:
        train_cls_flag = True
        train_reg_flag = default_regression

    logger.info("任务选择: 分类=%s, 回归=%s", '是' if train_cls_flag else '否', '是' if train_reg_flag else '否')

    train_models(
        df=df,
        model_save_dir=args.model_dir,
        train_classification=train_cls_flag,
        train_regression=train_reg_flag,
        classification_strategy=args.label_strategy,
        prediction_period=args.prediction_period,
        # ========== Stage 5: 特征选择参数 ==========
        ...
        reuse_feature_selection=args.reuse_feature_selection,
        force_refresh_feature_selection=args.force_refresh_feature_selection,
        feature_selection_cache_dir=args.feature_selection_cache_dir,
        feature_selection_cache_ttl_days=args.feature_selection_cache_ttl,
        # ========== Stage 5: Optuna超参数优化 ==========
