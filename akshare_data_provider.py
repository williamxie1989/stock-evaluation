...
class AkshareDataProvider:
    ...
    def get_all_stock_list(self):
        ...
        try:
            ...
            # 4. 获取北京证券交易所股票
            self.logger.info("正在获取北京证券交易所股票列表...")
            try:
                bj_stocks = self._retry_with_backoff(ak.stock_info_bj_name_code)
                if bj_stocks is not None and not bj_stocks.empty:
                ...
                self.logger.warning(f"获取北京证券交易所股票列表失败: {e}")
    ...
    def _classify_stock_market(self, code):
        ...
        # 北京证券交易所
            ...
            return {'market': 'BJ', 'board_type': '北交所', 'exchange': '北京证券交易所'}
